<?php
if (!function_exists('wd_blog_layout_function')) {
	function wd_blog_layout_function($atts) {
		...
		$wrap_class 		= $class .' '. $grid_list_class .' '.$columns_class.' '. $class_masonry;
		...
		<?php if( $special_blogs->have_posts() ) :?> 
			...
			<div id="<?php echo esc_html($random_id); ?>" 
				class="wd-blog-wrapper wd-blog-switchable-layout wd-shortcode-special-blog <?php echo esc_html($wrap_class); ?>">
...
if (!function_exists('wd_blog_layout_vc_map')) {
	function wd_blog_layout_vc_map() {
		return array(
