...
if (!class_exists('WD_Compress_Images')) {
	class WD_Compress_Images {
		...
		public function add_actions() {
			...
			add_action('wp_handle_upload', array($this, 'handle_upload_callback'));
		}

		public function handle_upload_callback( $data ) {
			$jpg_image_quality = 30; // 1-100
			$png_image_quality = 4; // 0-9
			$file_path = $data['file'];
			$image = false;
			...
			switch ( $data['type'] ) {
				case 'image/jpeg': {
					...
					@imagejpeg( $image, $file_path, $jpg_image_quality );
					break;          
				}
		
				case 'image/png': {
					//$image = imagecreatefrompng( $file_path );
					//@imagepng( $image, $file_path, $png_image_quality );
					break;          
				}
		...
		function upload_resize ($image_data){
			resize_error_log("**-start--resize-image-upload");
			if($this->image_re_compression) {
				...
				if(empty($image_data['file']) || empty($image_data['type'])) {
					resize_error_log("--non-data-in-file-( ".print_r($image_data, true)." )");	
					$fatal_error_reported = true;
				} else if(!in_array($image_data['type'], $valid_types)) {
					resize_error_log("--non-image-type-uploaded-( ".$image_data['type']." )");
					$fatal_error_reported = true;
				}
				resize_error_log("--filename-( ".$image_data['file']." )");
				$image_editor = wp_get_image_editor($image_data['file']);
				$image_type = $image_data['type'];

				if($fatal_error_reported || is_wp_error($image_editor)) {
					resize_error_log("--wp-error-reported");
				} else {
					$to_save = false;
					$resized = false;
					if($this->allow_image_resize) {
						resize_error_log("--resizing-enabled");
						...
						if((isset($sizes['width']) && $sizes['width'] > $this->max_width) || (isset($sizes['height']) && $sizes['height'] > $this->max_height)) {
							...
							$to_save = true;
							$sizes = $image_editor->get_size();
							resize_error_log("--new-size--".$sizes['width']."x".$sizes['height']);
						}
						else {
							resize_error_log("--no-resizing-needed");
						}
					} else {
						resize_error_log("--no-resizing-requested");
					}

					if($this->image_re_compression && ($image_type=='image/jpg' || $image_type=='image/jpeg')) {
						$to_save = true;
						resize_error_log("--compression-level--q-".$this->compression_level);
					} elseif(!$resized) {
						resize_error_log("--no-forced-recompression");
					}

					if($to_save) {
						$image_editor->set_quality($this->compression_level);
						$saved_image = $image_editor->save($image_data['file']);
						resize_error_log("--image-saved");
					} else {
						resize_error_log("--no-changes-to-save");
					}
				}
			} else {
				resize_error_log("--no-action-required");
			}
			resize_error_log("**-end--resize-image-upload\n");
			return $image_data;
		}

		function resize_error_log($message) {
			global $DEBUG_LOGGER;
			if($DEBUG_LOGGER) {
