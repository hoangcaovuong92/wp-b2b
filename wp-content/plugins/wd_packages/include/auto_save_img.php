...
if (!class_exists('WD_Autosave_Images')) {
	class WD_Autosave_Images {
		/**
		...
		public function fsockopen_image_header($image_url, $mode='Content-Type') {
		...
		public function save_images($image_url, $post_id){
			$save_images_filename = 'post_slug'; //post_slug_and_id
			set_time_limit(0);
			$file=file_get_contents($image_url);
			if ( $file ) {
				...
				if ($save_images_filename == 'post_slug' ) {
					$slug_name = $postname;
					$img_name = $slug_name.$match[2];
				} elseif ($save_images_filename == 'post_slug_and_id' ) {
		...
		public function insert_attachment($file, $id){
			$dirs = wp_upload_dir();
	...
	WD_Autosave_Images::get_instance();  // Start an instance of the plugin class 
}