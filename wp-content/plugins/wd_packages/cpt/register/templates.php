<?php
if (!class_exists('WD_Templates_Post_Type')) {
	class WD_Templates_Post_Type extends WD_Package_Metabox{
		/**
		...
		protected $post_type 				= 'wd_template';
		protected $post_type_single_name 	= 'Templates';
		protected $post_type_multiple_name 	= 'Templates';
		protected $post_type_display_name 	= 'Templates';
		protected $post_type_icon 			= 'dashicons-tagcloud';
		protected $show_in_nav_menus 		= false;
		protected $show_in_menu 			= 'wd-post-type-panel';
		protected $post_type_position		= 20;
		protected $taxonomy 				= 'wd_template_group';
		protected $taxonomy_single_name		= 'Templates Group';
		...
		protected function constant(){
			define('WD_TEMPLATES_POST_TYPE_BASE'		,   plugin_dir_path( __FILE__ ) );
			define('WD_TEMPLATES_POST_TYPE_BASE_URI'	,   plugins_url( '', __FILE__ ) );
		}
		...
		public function create_shortcode_string( $post_id ) {
			return "[$this->post_type post_id=$post_id]";
		}
		...
		public function init_admin_script($hook) {
			$screen = get_current_screen();
			if ($hook = 'post.php' && $this->check_current_screen()) {
	...
	WD_Templates_Post_Type::get_instance();  // Start an instance of the plugin class 
}