...
class Tax_Meta_Class {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme'])){
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_template_directory_uri() . '/framework/functions/metabox/libs/Tax-meta-class';
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.__('Remove', 'feellio').'" title="'.__('Remove', 'feellio').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.__('Add', 'feellio').'" title="'.__('Add', 'feellio').'" id="add-'.$field['id'].'"><br/></div>';
  ...
  public function show_field_image( $field, $meta ) {
    ...
    echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
    if ($has_image)
    echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".esc_attr__('Remove Image', 'feellio')."' type='button'/>";
    else
    echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".esc_attr__('Upload Image', 'feellio')."' type='button'/>";
    $this->show_field_end( $field, $meta );
  }
