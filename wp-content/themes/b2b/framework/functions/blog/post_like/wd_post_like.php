...
if (!class_exists('WD_Post_Like')) {
	class WD_Post_Like {
		...
		public function __construct(){
			...
			add_action('wd_hook_post_like', array($this, 'display_post_like' ));
		}
		
		public function enqueue_scripts() {
			wp_enqueue_script( 'wd-simple-likes-public-js', WD_THEME_FUNCTIONS_URI.'/blog/post_like/js/simple-likes-public.js', array( 'jquery' ), '0.5', false );
			wp_localize_script( 'wd-simple-likes-public-js', 'simpleLikes', array(
				'ajaxurl' => admin_url( 'admin-ajax.php' ),
				'like' => esc_html__( 'Like', 'feellio' ),
				'unlike' => esc_html__( 'Unlike', 'feellio' )
			) ); 
		}
		/**
		...
		public function process_simple_like() {
			...
			if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
				exit( esc_html__( 'Not permitted', 'feellio' ) );
			}
		...
		public function get_simple_likes_button( $post_id, $is_comment = NULL ) {
			...
			if ( $this->already_liked( $post_id, $is_comment ) ) {
				...
				$title = esc_html__( 'Unlike', 'feellio' );
				$icon = $icon_full;
			} else {
				$class = '';
				$title = esc_html__( 'Like', 'feellio' );
				$icon = $icon_empty;
			}
		...
		public function display_post_like() {
			echo $this->get_simple_likes_button( get_the_ID(), 0 );
		} 
		...
		public function get_like_count( $like_count ) {
			$like_text = esc_html__( '0', 'feellio' );
		...
		public function show_user_likes( $user ) { ?>        
			<table class="form-table">
				<tr>
					<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'feellio' ); ?></label></th>
					<td>
					<?php
					...
					<?php else : ?>
					<p><?php esc_html_e( 'You do not like anything yet.', 'feellio' ); ?></p>
					<?php 
					endif; 
					wp_reset_postdata(); 
					?>
