...
if (!class_exists('WD_Comments')) {
	class WD_Comments {
		...
		public function __construct(){
			...
			/* Display Comments Template - use on comments.php */
			// do_action('wd_hook_display_comments_template');
			add_action('wd_hook_display_comments_template', array($this, 'comments_template'), 5);
		...
		public function display_comment_form_by_mode($comment_mode = 'wordpress'){
			if ($comment_mode == 'wordpress') {
				if (!is_singular('product' )) {
					comments_template(''); //load from file comments.php
		...
		//replace comment template (comments.php - comments_template())
		public function comments_template(){
			...
			$comment_tabs		= array(
				'comments_list'	=> array(
						...
						'callback' 	=> array($this, 'comment_list'),
						'text'		=> $comments_list_text,
					),
				'comment_form'	=> array(
						'status'	=> comments_open(),
						'callback' 	=> array($this, 'comment_form'),
